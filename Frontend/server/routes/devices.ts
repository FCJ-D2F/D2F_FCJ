 ...
 */
export const handleGetDevices: RequestHandler = async (req, res) => {
  try {
 ...
 * Get device details
 */
export const handleGetDevice: RequestHandler = async (req, res) => {
  try {
    const { id } = req.params;
 ...
 */
export const handleGetDeviceHistory: RequestHandler = async (req, res) => {
  try {
    const { id } = req.params;
    const { startTime, endTime, limit = "100" } = req.query;

    const params = new URLSearchParams();
    if (startTime) params.append("startTime", startTime as string);
    if (endTime) params.append("endTime", endTime as string);
 ...
 */
export const handleDeviceCommand: RequestHandler = async (req, res) => {
  try {
    const { id } = req.params;
